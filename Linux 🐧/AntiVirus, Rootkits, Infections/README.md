...
# One liner to check if vulnerable to BPFDoor malware. https://blog.qualys.com/vulnerabilities-threat-research/2022/08/01/heres-a-simple-script-to-detect-the-stealthy-nation-state-bpfdoor
pid=$(grep packet_recvmsg /proc/*/stack | awk -F':' '{print $1}' | grep -o '[0-9]\+') && lsof -RPnl | grep SOCK_RAW | grep IP | grep "$pid" > /dev/null 2>&1 && echo "Compromised" || echo "Not compromised"
````
